...
<head>
    ...
    <link href="{{ asset('assets/css/themes/layout/aside/dark.cs') }}" rel="stylesheet" type="text/css" />
    <!--end::Layout Themes-->
...
<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
                ...
                <div class="d-flex flex-row-fluid flex-column justify-content-between">
                    ...
                    <div class="d-flex flex-column-fluid flex-column flex-center mt-5 mt-lg-0">
                        ...
                        <div class="login-form login-signin">
                            ...
                            @if(isset($errors))
                            <!-- Show global error (first error) if any -->
                            @if ($errors->any())
                            ...
                            <form method="POST" action="{{ route('login') }}" class="form" id="kt_login_signin_form">
                                ...
                                <div class="form-group py-3 m-0">
                                    <input class="form-control h-auto border-0 px-0 placeholder-dark-75" type="email" placeholder="Email" name="email" autocomplete="off" value="{{ old('email') }}" />
                                    @if(isset($errors))
                                    @if ($errors->has('email'))
                    ...
                    <div class="d-flex flex-column-auto justify-content-between mt-15">
                        <div class="text-dark-50 font-weight-bold order-2 order-sm-1 my-2">© {{ date('Y') }} GSC</div>
    ...
    <!--end::Page Scripts-->
    <script>
        document.getElementById('kt_login_signin_form').addEventListener('keydown', function(event) {
            if (event.key === 'Enter') {
                event.preventDefault(); // يمنع السلوك الافتراضي فقط عند الحاجة
                this.submit(); // يرسل النموذج
            }
        });
