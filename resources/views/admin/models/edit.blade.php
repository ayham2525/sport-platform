...
@section('page_title')
<h5 class="text-dark font-weight-bold my-2 mr-5">{{ __('titles.edit_model') }}</h5>
@endsection
...
<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
    ...
    <li class="breadcrumb-item">
        <span class="text-muted">{{ __('titles.edit_model') }}</span>
    </li>
</ul>
@endsection
    ...
    <div class="container">
        <div class="card card-custom">
            <div class="card-header">
                <h3 class="card-title">{{ __('titles.edit_model') }}</h3>
            </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                value="{{ old('name', $model->name) }}" placeholder="{{ __('columns.name') }}">
                            @error('name')
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror"
                                value="{{ old('slug', $model->slug) }}" placeholder="{{ __('columns.slug') }}">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select name="system_id" class="form-control @error('system_id') is-invalid @enderror">
                                ...
                                @foreach($systems as $system)
                                    <option value="{{ $system->id }}"
                                        {{ old('system_id', $model->system_id) == $system->id ? 'selected' : '' }}>
                                        {{ $system->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <select name="only_admin" class="form-control @error('only_admin') is-invalid @enderror">
                                <option value="0" {{ old('only_admin', $model->only_admin) == '0' ? 'selected' : '' }}>
                                    {{ __('columns.no') }}</option>
                                <option value="1" {{ old('only_admin', $model->only_admin) == '1' ? 'selected' : '' }}>
                                    {{ __('columns.yes') }}</option>
                            </select>
                            @error('only_admin')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea name="description" rows="3"
                                ...
                                placeholder="{{ __('columns.description') }}">{{ old('description', $model->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
