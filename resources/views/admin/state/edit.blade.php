...
@section('page_title')
    <h5 class="text-dark font-weight-bold my-2 mr-5">{{ __('state.edit') }}: {{ $state->name }}</h5>
@endsection
...
<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
    ...
    <li class="breadcrumb-item">
        <span class="text-muted">
            <i class="la la-edit mr-1"></i> {{ __('state.edit') }}
        </span>
    </li>
</ul>
@endsection
    ...
    <div class="container">
        <div class="card card-custom">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="la la-map mr-2"></i> {{ __('state.edit') }}: {{ $state->name }}
                </h3>
            </div>

            <form action="{{ route('admin.states.update', $state->id) }}" method="POST" class="form">
                @csrf
                @method('PATCH')
                ...
                <div class="card-footer text-right">
                    <button type="submit" class="btn btn-primary mr-2">
                        <i class="la la-check-circle mr-1"></i> {{ __('actions.update') }}
                    </button>
