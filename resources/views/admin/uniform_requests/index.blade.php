            ...
            <table class="table table-bordered table-hover table-responsive-sm">
                <thead>
                    <tr>
                        ...
                        <th><i class="la la-edit text-muted mr-1"></i> {{ __('uniform_requests.actions.edit') }} /
                            <i class="la la-trash text-muted mr-1"></i> {{ __('uniform_requests.actions.delete') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($uniformRequests as $index => $req)
                    <tr class="request-row-{{ $req->id }}">
                        ...
                        <td>{{ $req->size }}</td>
                        <td>
                            <span style="display: inline-block; width: 20px; height: 20px; background-color: {{ $req->color }}; border: 1px solid #ccc; border-radius: 4px;"></span>
                        </td>
                        <td>{{ $req->quantity }}</td>
                        <td>{{ $req->amount }}</td>
                        <td>{{ optional($req->currency)->code ?? 'N/A' }}</td>
                        <td>
                            <span class="badge badge-{{ $req->status === 'delivered' ? 'success' : ($req->status === 'ordered' ? 'info' : 'secondary') }}">
                                {{ __('uniform_requests.statuses.' . $req->status) }}
                            </span>
                        </td>
                        <td>{{ $req->admin_remarks ?? '-' }}</td>
                        <td>
                            {{ $req->approved_at ? \Carbon\Carbon::parse($req->approved_at)->format('Y-m-d H:i') : '-' }}
                        </td>
                        <td>
                            {{ $req->ordered_at ? \Carbon\Carbon::parse($req->ordered_at)->format('Y-m-d H:i') : '-' }}
                        </td>
                        <td>
                            {{ $req->delivered_at ? \Carbon\Carbon::parse($req->delivered_at)->format('Y-m-d H:i') : '-' }}
                        </td>
                        <td>
                            {{ $req->created_at ? \Carbon\Carbon::parse($req->created_at)->format('Y-m-d') : '-' }}
                        </td>
                        <td>
                    ...
                    @empty
                    <tr>
                        <td colspan="15" class="text-center text-muted">{{ __('uniform_requests.no_data') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    , success: function() {
                        ...
                        const status = $('.request-row-' + requestId).data('status'); // get the row's status
                        const countElement = $('#status-count-' + status);
                        if (countElement.length) {
                            const newCount = Math.max(0, parseInt(countElement.text()) - 1);
