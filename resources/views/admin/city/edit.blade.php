...
@section('page_title')
    <h5 class="text-dark font-weight-bold my-2 mr-5">
        <i class="la la-building-o mr-1"></i> {{ __('city.edit_title') }}: {{ $city->name }}
    </h5>
@endsection
...
<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
    ...
    <li class="breadcrumb-item">
        <span class="text-muted">
            <i class="la la-edit mr-1"></i> {{ __('city.edit_title') }}
        </span>
    </li>
</ul>
@endsection
    ...
    <div class="container">
        <div class="card card-custom">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="la la-edit mr-1"></i> {{ __('city.edit_title') }}: {{ $city->name }}
                </h3>
            </div>

            <form action="{{ route('admin.cities.update', $city->id) }}" method="POST" class="form">
                @csrf
                @method('PATCH')
