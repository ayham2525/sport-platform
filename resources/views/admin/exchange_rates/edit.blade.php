...
@section('breadcrumb')
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item">
            <span class="text-muted">{{ __('exchange_rate.titles.edit') }}</span>
        </li>
    </ul>
@endsection
...
@section('content')
    <div class="card card-custom">
        <div class="card-body">
            ...
            <form id="exchange-form" method="POST" action="{{ route('admin.exchange-rates.update', $exchangeRate->id) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select name="base_currency" class="form-control" required>
                            ...
                            @foreach ($currencies as $currency)
                                <option value="{{ $currency->code }}" @if ($currency->code == $exchangeRate->base_currency) selected @endif>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select name="target_currency" class="form-control" required>
                            <option value="">{{ __('exchange_rate.actions.select') }}</option>
                            @foreach ($currencies as $currency)
                                <option value="{{ $currency->code }}" @if ($currency->code == $exchangeRate->target_currency) selected @endif>
                                    {{ $currency->code }} - {{ $currency->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input type="number" step="0.0001" name="rate" class="form-control"
                            value="{{ $exchangeRate->rate }}" required>
                    </div>

                    <div class="form-group col-md-4">
                        <label>{{ __('exchange_rate.fields.fetched_at') }}</label>
                        <input type="datetime-local" name="fetched_at" class="form-control"
                            value="{{ \Carbon\Carbon::parse($exchangeRate->fetched_at)->format('Y-m-d\TH:i') }}" required>
                    </div>
                </div>

                <div class="text-right">
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save mr-1"></i> {{ __('exchange_rate.actions.update') }}
                    </button>
