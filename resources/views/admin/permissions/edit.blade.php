...
@section('page_title')
<h5 class="text-dark font-weight-bold my-2 mr-5">{{ __('actions.edit') }} {{ __('titles.permission') }}</h5>
@endsection
...
<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
    ...
    <li class="breadcrumb-item">
        <span class="text-muted">{{ __('actions.edit') }}</span>
    </li>
</ul>
@endsection
    ...
    <div class="container">
        <div class="card card-custom">
            <div class="card-header">
                <h3 class="card-title">{{ __('actions.edit') }} {{ __('titles.permission') }}</h3>
            </div>
            <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <select name="system_id" id="system_id" class="form-control @error('system_id') is-invalid @enderror">
                                ...
                                @foreach ($systems as $system)
                                    <option value="{{ $system->id }}" {{ old('system_id', optional($permission->role->system)->id) == $system->id ? 'selected' : '' }}>
                                        {{ $system->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <select name="role_id" id="role_id" class="form-control @error('role_id') is-invalid @enderror">
                                <option value="{{ $permission->role_id }}">{{ $permission->role->name }}</option>
                            </select>
                            @error('role_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <select name="model_id" class="form-control @error('model_id') is-invalid @enderror">
                                ...
                                @foreach ($models as $model)
                                    <option value="{{ $model->id }}" {{ old('model_id', $permission->model_id) == $model->id ? 'selected' : '' }}>
                                        {{ $model->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            ...
                            <select name="action[]" id="action-select" class="form-control @error('action') is-invalid @enderror" multiple>
                                @foreach ($actions as $action)
                                    ...
                                    @endphp
                                    <option value="{{ $action }}" data-icon="{{ $icon }}"
                                        {{ in_array($action, old('action', [$permission->action])) ? 'selected' : '' }}>
                                        {{ ucfirst($action) }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="card-footer">
                    <button type="submit" class="btn btn-primary mr-2">
                        <i class="la la-check"></i> {{ __('actions.update') }}
                    </button>
...
<script>
    ...
    document.getElementById('select-all-actions').addEventListener('change', function () {
        const options = $actionSelect.find('option');
        options.prop('selected', this.checked);
